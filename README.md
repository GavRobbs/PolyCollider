...
An intersection between two polygons is defined by the vertices of, or area delimited by one polygon touching or being inside the boundaries of another polygon.
...
Support functions have a somewhat complicated mathematical definition, but for our purposes, we can skip them here. It is sufficient for us to say that a support function acts on a polygon, and takes a direction vector as an input, and returns the vertex on the polygon furthest in that direction.

Different geometrical shapes have different support functions, but the general idea is to take the dot product of all the vertices that define the edge of the polygon with the direction vector, and pick the vertex with the largest result.

Sometimes, when trying to find the support function, two or more vertices will have the same dot product. You just need to pick one, it shouldn't matter too much which one you pick.
